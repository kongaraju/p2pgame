...
var pcDataConstraints={optional: [{RtpDataChannels: true}]};
...
function initialize() {
  ...
  //var heads=document.querySelectorAll(".dataHead").bind(document);
  var forEach=Array.prototype.forEach;
 var $$ = document.querySelectorAll.bind(document);
  forEach.call($$(".dataHead"),function(el){
...
function createPeerConnection() {
  try {
    ...
    pc = new RTCPeerConnection(pcConfig, pcDataConstraints);
    ...
    console.log('Created RTCPeerConnnection with:\n' +
                ...
                '  constraints: \'' + JSON.stringify(pcDataConstraints) + '\'.');
  } catch (e) {
  ...
  pc.ondatachannel = onDataChannelRecieved;
}

function onDataChannelRecieved(e){
	recieveChannel=e.channel;
	setupDataChannel(recieveChannel);
}
function setupDataChannel(channel){
	channel.onmessage=onDataChannelMsgRecieved;
...
function onDataChannelStateChange(e){
	...
	//input.addEventListener("keydown",sendData,false);
	 Peer.onDataChannelOpened.call(Peer);
	}else if(sendChannel.readyState=="close") {
	 // document.getElementById("dataContainer").style.display="none";
	 // input.removeEventListener("keydown",sendData);
...
function onDataChannelMsgRecieved(e){
	Peer.onmessage.call(Peer,e.data);

	console.log("Data msg Rcvd: "+e.data);
	document.getElementById("convWindow").innerHTML= e.data+"<br>";
...
function sendData(e){
	e.stopPropagation();
	if(e.keyCode==13){
	  sendChannel.send(e.currentTarget.value);
...
/*
Peer Object for Game
*/

var dataPeer=function(){
	this._events={};
	this.starter=false;
	this.onDataChannelOpened="";//callback on data channel open
}
dataPeer.prototype.init=function(){
	
}

dataPeer.prototype.emit=function(cmd,data){
	var msg={};
	  msg.cmd=cmd;
	  msg.data=data;
...
dataPeer.prototype.onmessage=function(_msg){
	var msg=JSON.parse(_msg);
	//console.log(Peer);
	//console.log(msg);
	Peer._events[msg.cmd].call(this,msg.data);
	
}
dataPeer.prototype.disconnect=function(){
	Peer._events['disconnect'].call(this);
}
dataPeer.prototype.on = function(event,callback) {
        this._events[event]=callback;
        if(event=='connect')
        	callback.call(this);
    };
var Peer=new dataPeer;
...
function transitionToWaiting() {
  ...
  //resetStatus();
  setStatus(" Refresh to play again.")
}
