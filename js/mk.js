;(function () {
    ...
    mk.controllers.Network.prototype._endGame=function(){
    	clearInterval(this.actionUpdater);
    ...
    mk.controllers.Network.prototype._addSocketHandlers = function () {
        ...
        this.lifeUpdater = setInterval(function () {
            self._transport.emit(m.LIFE_UPDATE, f.getLife());
        }, 2000);
        
        this.actionUpdater = setInterval(function () {
            if (!f.isJumping()) {
    ...
    mk.controllers.Network.prototype._initialize = function () {
     ...
     mk.controllers.Peer = function (options) {
        mk.controllers.Basic.call(this, options);
        this._isHost = options.isHost;
        this._gameName = options.gameName;
        this._transport = options.transport || this.Transports.socketio;
    ...
    mk.controllers.Peer.prototype = new mk.controllers.Basic();

    mk.controllers.Peer.prototype.Requests = {
        CREATE_GAME : 'create-game',
        JOIN_GAME   : 'join-game'
    };

    mk.controllers.Peer.prototype.Responses = {
        SUCCESS         : 0,
        GAME_EXISTS     : 1,
        GAME_NOT_EXISTS : 2,
        GAME_FULL       : 3
    ...
    mk.controllers.Peer.prototype.Messages = {
        EVENT            : 'event',
        LIFE_UPDATE      : 'life-update',
        POSITION_UPDATE  : 'position-update',
        PLAYER_CONNECTED : 'player-connected'
    ...
    mk.controllers.Peer.prototype.Transports = {
        socketio: {}
    };

    mk.controllers.Peer.prototype.Transports.socketio.init = function() {
        this._socket = io.connect();
    };

    mk.controllers.Peer.prototype.Transports.socketio.emit = function() {
        this._socket.emit.apply(this._socket, arguments);
    };

    mk.controllers.Peer.prototype.Transports.socketio.on = function() {
        this._socket.on.apply(this._socket, arguments);
    };

    mk.controllers.Peer.prototype._createGame = function (game) {
        this._transport.emit(this.Requests.CREATE_GAME, this._gameName);
        this._addSocketHandlers();
    };

    mk.controllers.Peer.prototype._addSocketHandlers = function () {
        var opponent = this.fighters[+!this._player],
            f = this.fighters[this._player],
            m = this.Messages,
            self = this;
    ...
    mk.controllers.Peer.prototype._moveFighter = function (f, m) {
        if (m) {
            this._transport.emit('event', m);
            f.setMove(m);
        }
    ...
    mk.controllers.Peer.prototype._joinGame = function (game) {
        this._transport.emit(this.Requests.JOIN_GAME, this._gameName);
        this._addSocketHandlers();
    };

    mk.controllers.Peer.prototype._initialize = function () {
        var self = this;
        if (this._isHost) {
            this._player = 1;
        } else {
    ...
    mk.start = function (options) {
        ...
        switch (type) {
            ...
            case 'webcaminput':
                ...
                break;
	    case 'peer':
		mk.game = new mk.controllers.Peer(options);
		break;
            default:
                mk.game = new mk.controllers.Basic(options);
        }
